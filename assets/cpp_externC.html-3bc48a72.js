...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>可以看到add生成符号是_Z3addii。</p><p>因此，加与不加<code>extern &quot;C&quot;</code>，add函数生成的符号名称是不同的。</p><p>看到这里，聪明的你已经大概知道<code>extern &quot;C&quot;</code>的作用了，就是修改了符号表的生成方式，将c++符号的生成方式换成了c的生成方式。</p><p>c库中生成的符号是c编译器的符号， 因此c语言可以直接链接。而c++程序需要使用<code>extern &quot;C&quot;</code>让编译器使用c的符号命名方式去进行链接，这样才能找到对应的符号。</p><h2 id="c-写的库给c-c-调用" tabindex="-1"><a class="header-anchor" href="#c-写的库给c-c-调用" aria-hidden="true">#</a> c++写的库给c/c++调用</h2><p>下面这个例子，我们使用c++语言构建了一个add函数，并提供了其头文件。我们要将该实现提供给c和c++的程序调用。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token builtin class-name">.</span>
├── add.cpp
├── add.h
├── main_c.c
├── main_cpp.cpp
...
<span class="token target symbol">clean</span><span class="token punctuation">:</span>
        rm -f *.o main_cpp main_c
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>其实这种场景和第一种场景是基本一致的。现在我们的库add.o是使用c++编译器生成的。</p><p>我们使用readelf查看其内容，可以看到其内容和之前c语言生成的库，add函数的符号是一样的。因为我们此时编译时使用了<code>extern &quot;C&quot;</code>,也就是说使用c语言的符号构建方式进行编译。</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>Symbol table <span class="token string">&#39;.symtab&#39;</span> contains <span class="token number">9</span> entries:
   Num:    Value          Size Type    Bind   Vis      Ndx Name
     <span class="token number">0</span>: 0000000000000000     <span class="token number">0</span> NOTYPE  LOCAL  DEFAULT  UND
     <span class="token number">1</span>: 0000000000000000     <span class="token number">0</span> FILE    LOCAL  DEFAULT  ABS add.c
     <span class="token number">2</span>: 0000000000000000     <span class="token number">0</span> SECTION LOCAL  DEFAULT    <span class="token number">1</span>
     ...
     <span class="token number">7</span>: 0000000000000000     <span class="token number">0</span> SECTION LOCAL  DEFAULT    <span class="token number">4</span>
     <span class="token number">8</span>: 0000000000000000    <span class="token number">20</span> FUNC    GLOBAL DEFAULT    <span class="token number">1</span> <span class="token function">add</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>接下来链接的过程就和第一个场景一样了。</p><p>在本场景中，使用c++编写了一个包含add函数的模块，对其编译时，使用了c语言的符号构建方式。因此其符号表和c语言的库是相同的。最终链接时，由于加上了<code>extern &quot;C&quot;</code>， 链接过程也将使用c语言的方式去寻找符号。</p><h2 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h2><ul><li>extern &quot;C&quot;实际上就是告诉c++编译器去使用c编译器的规则去进行构建。</li><li>在日常使用中，c++调用c库的使用频率更高一些，案例一就是这样的例子。</li></ul>`,56),c=[p];function l(d,t){return s(),a("div",null,c)}const u=n(i,[["render",l],["__file","cpp_externC.html.vue"]]);export{u as default};

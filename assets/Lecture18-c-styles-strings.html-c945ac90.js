import{_ as d,V as c,W as o,X as n,Y as s,$ as e,a0 as t,F as p}from"./framework-9a29aaa0.js";const i={},l=t(`<h1 id="第十八讲-c字符串" tabindex="-1"><a class="header-anchor" href="#第十八讲-c字符串" aria-hidden="true">#</a> 第十八讲 c字符串</h1><p>C风格字符串是一个字节数组，其中最后一个字节等于0, 我们可以遍历字符串直到找到0字节来计算出字符串的长度。在 C/C++ 中，这看起来像这样：</p><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token class-name">size_t</span> <span class="token function">strlen</span><span class="token punctuation">(</span><span class="token keyword">char</span><span class="token operator">*</span> s<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>注意<code>0</code>和字符常量<code>\\0</code>是完全相同的。</p><p>计算字符串的长度是所有字符串操作的基础。字符串的复制、追加、搜索等操作依赖于字符串的长度。因此，我们希望使此操作尽可能快。其他字符串操作如下所示:</p><ul><li><code>strcpy</code> – 将一个字符串复制到另一个字符串。</li><li><code>strcat</code> – 将两个字符串连接成第三个字符串。</li><li><code>strstr</code> – 在字符串中搜索子字符串</li></ul><p>上面的<code>strlen</code>的汇编版本如下所示:</p><div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code>strlen:
    ; rdi = char* s
    ; rax = returned length
    xor rax, rax
...
.return:
    ret
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>汇编版本的<code>strlen</code>没有比原始C版本的方法长很多，然而，计算长度时逐字节访问内存有些低效。后面，我们将介绍多种不同的方法来执行此操作。比如我们可以使用带有<code>rep</code>前缀字符串的指令进行加速。也可以使用<code>XMM</code>寄存器并行化的高级方法进行加速。</p><p><code>strcpy</code>的作用时将从一个字符串逐字节复制到另一个字符串。我们假设目标字符串有足够的空间。在C语言中，你可以这样实现：</p><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">char</span><span class="token operator">*</span> <span class="token function">strcpy</span><span class="token punctuation">(</span><span class="token keyword">char</span><span class="token operator">*</span> dest<span class="token punctuation">,</span> <span class="token keyword">char</span><span class="token operator">*</span> src<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们返回<code>dest</code>中最后一个终止符0的地址，因为它在下面实现<code>strcat</code>时很有用。</p><p>在汇编语言中，我们可以这样实现：</p><div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code>strcpy:
    ; rdi = char* dest
    ; rsi = char* src
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code>strcat</code>的汇编实现如下所示：</p><div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code>strcat:
  ; rdi = char* dest
  ; rsi = char* a
  ; rdx = char* b
  push rbp
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这种实现方式的时间复杂度是<code>O(mn)</code>，这里<code>m</code>是源字符串的长度， <code>n</code>是搜索字符串的长度。存在有更快的算法，但它们依赖于模式 ptn 的各种跳过表的预计算(KMP算法)。</p><p>为了将其转化为汇编代码，我们需要进行一些修改。首先，我们要消除对<code>strlen</code>的调用。应该可以检测循环中搜索字符串和模式的结尾：</p><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">char</span><span class="token operator">*</span> <span class="token function">strstr</span><span class="token punctuation">(</span><span class="token keyword">char</span><span class="token operator">*</span> src<span class="token punctuation">,</span> <span class="token keyword">char</span><span class="token operator">*</span> ptn<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>第二个调整是在函数内部，除了 <code>src + i</code> 之外，不使用 <code>src</code>。可以用基于指针的循环替换外部整数循环：</p><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">char</span><span class="token operator">*</span> <span class="token function">strstr</span><span class="token punctuation">(</span><span class="token keyword">char</span><span class="token operator">*</span> src<span class="token punctuation">,</span> <span class="token keyword">char</span><span class="token operator">*</span> ptn<span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token keyword">return</span> <span class="token number">0</span><span class="token punctuation">;</span> <span class="token comment">// Not found</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们可以类似地替换内部循环，只要我们保存原始的<code>ptn</code>，这样我们就可以从头开始：</p><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">char</span><span class="token operator">*</span> <span class="token function">strstr</span><span class="token punctuation">(</span><span class="token keyword">char</span><span class="token operator">*</span> src<span class="token punctuation">,</span> <span class="token keyword">char</span><span class="token operator">*</span> ptn<span class="token punctuation">)</span> <span class="token punctuation">{</span>
  <span class="token keyword">for</span><span class="token punctuation">(</span><span class="token punctuation">;</span> <span class="token punctuation">;</span> <span class="token operator">++</span>src<span class="token punctuation">)</span> <span class="token punctuation">{</span>
    bool matches <span class="token operator">=</span> true<span class="token punctuation">;</span>
  ...
  <span class="token keyword">return</span> <span class="token number">0</span><span class="token punctuation">;</span> <span class="token comment">// Not found</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>因为内循环中的字符串结束条件位于开始处，所以我们可以将其折叠到循环条件中。</p><p>最后，我们实际上不需要存储变量 matches 的值。对于内循环中的每次比较，如果两个字节不相等，则跳转到外循环的更新步骤。</p><p>另一方面，如果内部循环正常完成，这意味着所有字节都相等，我们可以返回。在这种情况下，程序集跳转到任意标签的能力实际上允许我们简化代码，删除标志变量。</p><p>这个函数很简单，我们可以将它转化成汇编代码。</p><div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code>strstr:
  ; rdi = char* src
  ; rsi = char* ptn
  ; rax = returned char*
...
.cont_src_loop:
  inc rax
  jmp .src_loop
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>为了进行<code>byte [rdi]</code> 与<code>byte [rsi]</code> 的比较，我们使用字符串指令<code>cmpsb</code>，它允许我们比较两个内存操作数，这是我们通常无法做到的。 cmps 是一系列字符串指令之一，所有这些指令都隐式使用 <code>rdi</code>/<code>rsi</code> 作为读取地址。</p><h2 id="字符串指令" tabindex="-1"><a class="header-anchor" href="#字符串指令" aria-hidden="true">#</a> 字符串指令</h2><p>有许多特定于字符串的指令，全部以<code>s</code> 结尾，它们具有几个共同的特征：</p><ul><li>它们都隐式使用 <code>rdi</code> 和 <code>rsi</code> 寄存器作为地址。它们要么用作两个源操作数（例如，用于字符串比较 <code>cmps</code>），要么用作类似传输操作的源（rsi）和目标（rdi）。</li><li>它们每次都会隐式增加 <code>rdi</code> 和 <code>rsi</code>。当重复最终终止时，<code>rdi</code> 和 <code>rsi</code> 保留在其最终位置。</li><li>它们都接受rep前缀，处理器本身将重复执行指令，我们需要写循环的代码。</li><li>它们允许字节、字、双字或四字的字符串。</li></ul><h3 id="rep前缀" tabindex="-1"><a class="header-anchor" href="#rep前缀" aria-hidden="true">#</a> <code>rep</code>前缀</h3><p><code>rep</code> 前缀是修饰符，可应用于少数指令，它会提示CPU在内部重复执行它们，直到满足某些条件。<code>rep</code>系列的前缀有下面这些：</p><ul><li><code>rep</code> – 重复 <code>rcx</code> 次，就像<code>loop</code>指令一样，重复执行直到 <code>rcx == 0</code>。所有其他<code>rep</code>前缀也隐式使用此条件；也就是说，<code>repe</code>的停止条件是如果 <code>[rdi] != [rsi]</code> 或 <code>rcx == 0</code>。</li><li><code>repe</code> – 重复直到 <code>[rdi] != [rsi]</code> 或在某些情况下，直到 <code>[rdi] != rax</code>。这基本上循环直到设置零标志。 <code>repz</code> 是此前缀的别名。</li><li><code>repne</code> – 重复直到 <code>[rdi] == [rsi]</code> 或在某些情况下，直到 <code>[rsi] == rax</code>。这基本上循环直到零标志被清除。 <code>repnz</code> 是此前缀的别名。</li></ul><p><code>rep</code> 前缀可以与字符串指令 <code>movs、lods、stos</code> 一起使用。 <code>repe</code>/<code>repne</code> 前缀可与字符串指令 <code>cmps</code> 和 <code>scas</code> 一起使用。</p><p>请注意，无论使用什么前缀，重复都会在 <code>rcx == 0</code> 时终止。</p><p>因此，如果您不希望 <code>rcx</code> 对指令重复次数产生任何影响，请将其设置为可能的最大无符号 qword 值：</p><p><code>mov rcx, -1</code></p><h3 id="传输指令-lods、stos-和-movs" tabindex="-1"><a class="header-anchor" href="#传输指令-lods、stos-和-movs" aria-hidden="true">#</a> 传输指令：<code>lods</code>、<code>stos</code> 和 <code>movs</code></h3><p>存在从内存到<code>al</code>、从<code>al</code>到内存以及从内存到内存传输数据的三个指令。这些是 <code>lods</code>、<code>stos</code> 和 <code>movs</code>：</p><table><thead><tr><th>指令</th><th>描述</th></tr></thead><tbody><tr><td><code>lodsb</code></td><td>将 <code>[rdi]</code>处的一个字节加载到 <code>al</code></td></tr><tr><td><code>stosb</code></td><td>将 <code>al</code> 中的字节写入字节 <code>[rdi]</code></td></tr><tr><td><code>movsb</code></td><td>对于旧的模式，<code>movsb</code>从<code>DS:(E)SI</code>拷贝一个字节到<code>ES:（E)DI</code>。对于64位模式， <code>movsb</code>从<code>[rsi]</code> 拷贝一个字节到 <code>[rdi]</code>。<code>movsb</code>操作之后会对<code>rsi</code>和<code>rdi</code>进行递增或者递减，递增还是递减由<code>EFLAGS</code>寄存器中的方向位(DF: direction flag)来决定， DF=0，则进行递增， DF=1，则进行递减。</td></tr></tbody></table><p><code>rep</code> 前缀使这些操作运行 <code>rcx</code> 次。其他前缀不能与它们一起使用，因此，必须提前知道输入字符串的长度。尽管如此，<code>stosb</code> 仍可用于用常量字节填充内存数组，而 <code>movsb</code> 则可用于将一个字符串复制到另一个字符串中。<code>rep lodsb</code> 并不是特别有用，因为它会重复地将字节加载到 al 中，但随后不会对它们执行任何操作。</p><h3 id="比较指令-cmps-和-scas" tabindex="-1"><a class="header-anchor" href="#比较指令-cmps-和-scas" aria-hidden="true">#</a> 比较指令：<code>cmps</code> 和 <code>scas</code></h3><p><code>cmpsb</code> 将 <code>[rdi]</code> 处的字节与 <code>[rsi]</code> 处的字节进行比较，并相应地更新标志。由于 <code>repe</code>/<code>repne</code> 前缀使用 <code>ZF</code>，因此可用于按字节比较一对字符串，在相等/不相等的第一对字节处停止。</p><p><code>scas</code> 将字节 <code>[rdi]</code> 与 <code>al</code> 进行比较并相应地更新标志。因此，虽然 <code>cmps</code> 对应于按字节比较两个字符串，但 <code>scas</code> 对应于在字符串中搜索存储在 <code>al</code> 中的特定字符。同样，<code>repe</code>/<code>repne</code> 可用于搜索第一次出现等于/不等于 <code>al</code> 的字节。</p><p>问题：我们可以使用repe前缀来简化上面strstr的实现吗？</p><p>就我个人而言，我不这么认为，因为我们的终止条件比仅仅 !=; 更复杂。我们还必须检查任一列表中的终止 \\0，但我很高兴被证明是错误的！</p><h3 id="字符串操作-strlen" tabindex="-1"><a class="header-anchor" href="#字符串操作-strlen" aria-hidden="true">#</a> 字符串操作 strlen</h3><p>我们可以使用 <code>scas</code> 来实现 <code>strlen</code> 的支持<code>rep</code>的版本</p><div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code>strlen:
    ; rdi = char*
    ; rax = return length
    ...
    ret
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="并行化strlen" tabindex="-1"><a class="header-anchor" href="#并行化strlen" aria-hidden="true">#</a> 并行化strlen</h3><p>我们当前版本的 <code>strlen</code> 一次仅加载一个字节。通过使用 64 位寄存器的全宽度，我们可以在相同的时间内加载 8 个字节，但是我们会遇到在这 8 个字节内的任何位置检测到单个 0 字节的问题。最简单的方法是重复移位并检查寄存器的低/高字节部分：</p><div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code> mov rbx, qword [rdi]
  cmp bl, 0
  je .low
  cmp bh, 0
  je .high
  ...
  cmp bl, 0
  ...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>因为有四个字，所以我们需要执行四次移位/比较过程（如果我们手动将它们全部写出来，而不是使用循环，速度会更快）。主要的问题是我们使用通用寄存器（rbx），就像它是一个 8 字节的向量寄存器一样，但事实并非如此。如果我们改用 xmm 寄存器，我们就有更多的指令可供使用，可以平等地对待所有字节。</p>`,59),r={href:"https://www.agner.org/optimize/#asmlib",target:"_blank",rel:"noopener noreferrer"},u=t(`<div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code>    ; rdi = char* s

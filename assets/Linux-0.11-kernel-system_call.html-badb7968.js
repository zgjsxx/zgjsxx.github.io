import{_ as s,V as n,W as e,a0 as a}from"./framework-9a29aaa0.js";const t={},i=a(`<ul><li><a href="#linux-011-kernel%E7%9B%AE%E5%BD%95%E8%BF%9B%E7%A8%8B%E7%AE%A1%E7%90%86system_calls%E8%AF%A6%E8%A7%A3">Linux-0.11 kernel目录进程管理system_call.s详解</a><ul><li><a href="#%E6%A8%A1%E5%9D%97%E7%AE%80%E4%BB%8B">模块简介</a></li><li><a href="#%E8%BF%87%E7%A8%8B%E5%88%86%E6%9E%90">过程分析</a><ul><li><a href="#system_call">system_call</a></li><li><a href="#ret_from_sys_call">ret_from_sys_call</a></li><li><a href="#sys_fork">sys_fork</a></li><li><a href="#coprocessor_error">coprocessor_error</a></li><li><a href="#device_not_available">device_not_available</a></li><li><a href="#timer_interrupt">timer_interrupt</a></li><li><a href="#hd_interrupt">hd_interrupt</a></li><li><a href="#floppy_interrupt">floppy_interrupt</a></li></ul></li></ul></li><li><a href="#do_floppy%E4%B8%BA%E4%B8%80%E5%87%BD%E6%95%B0%E6%8C%87%E9%92%88%E5%B0%86%E8%A2%AB%E8%B5%8B%E5%80%BC%E5%AE%9E%E9%99%85%E5%A4%84%E7%90%86c%E5%87%BD%E6%95%B0%E6%8C%87%E9%92%88%E8%AF%A5%E6%8C%87%E9%92%88%E5%9C%A8%E8%A2%AB%E4%BA%A4%E6%8D%A2%E6%94%BE%E5%88%B0eax%E5%AF%84%E5%AD%98%E5%99%A8%E5%90%8E">do_floppy为一函数指针，将被赋值实际处理C函数指针。该指针在被交换放到eax寄存器后</a></li><li><a href="#%E5%B0%B1%E5%B0%86do_floppy%E5%8F%98%E9%87%8F%E7%BD%AE%E7%A9%BA%E7%84%B6%E5%90%8E%E6%B5%8B%E8%AF%95eax%E4%B8%AD%E5%8E%9F%E6%8C%87%E9%92%88%E6%98%AF%E5%90%A6%E4%B8%BA%E7%A9%BA%E8%8B%A5%E6%98%AF%E5%88%99%E4%BD%BF%E6%8C%87%E9%92%88%E6%8C%87%E5%90%91c%E5%87%BD%E6%95%B0">就将do_floppy变量置空。然后测试eax中原指针是否为空，若是则使指针指向C函数。</a> - <a href="#parallel_interrupt">parallel_interrupt</a></li></ul><h1 id="linux-0-11-kernel目录进程管理system-call-s详解" tabindex="-1"><a class="header-anchor" href="#linux-0-11-kernel目录进程管理system-call-s详解" aria-hidden="true">#</a> Linux-0.11 kernel目录进程管理system_call.s详解</h1><h2 id="模块简介" tabindex="-1"><a class="header-anchor" href="#模块简介" aria-hidden="true">#</a> 模块简介</h2><p>本节主要介绍了在Linux-0.11中关于系统调用的相关实现。Linux-0.11使用<code>int 0x80</code>中断以及<code>eax</code>寄存器中存储的功能号去调用内核中所提供的功能，在系统调用发生的过程中伴随着用户态向内核态的主动切换。</p><p>需要注意的时，用户通常并不是直接使用系统调用的中断，而是libc中所提供的接口函数实现。</p><p>系统调用处理过程的整个流程如下图所示：</p><figure><img src="https://github.com/zgjsxx/static-img-repo/raw/main/blog/Linux/kernel/Linux-0.11/Linux-0.11-kernel/system_call/syscall_flow.png" alt="系统调用" tabindex="0" loading="lazy"><figcaption>系统调用</figcaption></figure><h2 id="过程分析" tabindex="-1"><a class="header-anchor" href="#过程分析" aria-hidden="true">#</a> 过程分析</h2><h3 id="system-call" tabindex="-1"><a class="header-anchor" href="#system-call" aria-hidden="true">#</a> system_call</h3><p>当0x80号中断发生的时候，CPU除了切入内核态之外，还会自动完成下列几件事：</p><p>1.找到当前进程的内核栈, 通过<code>tss</code>中的<code>esp0</code> <code>ss0</code>定位</p><p>2.在内核栈中依次压入用户态的寄存器<code>SS</code>、<code>ESP</code>、<code>EFLAGS</code>、<code>CS</code>、<code>EIP</code></p><p>当内核从系统调用中返回的时候，需要调用<code>iret</code>指令来返回用户态，显然iret代表的是内核栈中一系列的寄存器<code>SS</code>、<code>ESP</code>、<code>EFLAGS</code>、<code>CS</code>、<code>EIP</code>弹出操作。</p><p>在<code>system_call</code>中会将DS、ES、FS、EDX、ECX、EBX入栈。</p><p>在调用<code>sys_call</code>函数时，会将系统调用号传给<code>eax</code>， 因此首先判断<code>eax</code>是否超过了最大的系统调用号, 如果超出了，就跳到 <code>bad_sys_call</code> 标签处处理错误。</p><div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code>cmpl $nr_system_calls-1,%eax
ja bad_sys_call
...
<span class="token comment">/* these are hardcoded - don&#39;t touch */</span>
	...
	addl $20,%esp
1:	ret
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code>sys_fork</code>首先调用<code>find_empty_process</code>去进程<code>task_struct</code>数组中寻找一个空位，如果寻找不到就直接返回。如果寻找到了，就将一些寄存器压栈，进而调用<code>copy_process</code>方法。在调用<code>sys_fork</code>方法时，内核栈的状态如下所示：</p><figure><img src="https://github.com/zgjsxx/static-img-repo/raw/main/blog/Linux/kernel/Linux-0.11/Linux-0.11-kernel/fork/system_call_stack.png" alt="内核栈的状态" tabindex="0" loading="lazy"><figcaption>内核栈的状态</figcaption></figure><h3 id="coprocessor-error" tabindex="-1"><a class="header-anchor" href="#coprocessor-error" aria-hidden="true">#</a> coprocessor_error</h3><p>这段代码是一个处理协处理器错误的中断处理程序。</p><div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code>coprocessor_error:
	push %ds
	push %es
	push %fs
	pushl %edx
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>函数接受了一系列参数，包括寄存器的值和当前的代码段、指令指针等信息。</p><p>首先，它检查当前的代码段是否为用户代码空间（<code>0x000F</code>）。如果不是，则打印错误消息并引发 panic。这表示内核中出现了需要数学模拟的情况，这通常是不允许的，因此会导致内核崩溃。</p><p>如果当前代码段确实为用户代码空间，它会依次从指令指针处读取两个字节的数据，即模拟执行用户空间的指令。然后，它打印所读取的指令的地址、内容。</p><p>最后，它将当前任务的信号字段中的 <code>SIGFPE</code> 位设置为1，表示发生了浮点运算错误。</p><h3 id="timer-interrupt" tabindex="-1"><a class="header-anchor" href="#timer-interrupt" aria-hidden="true">#</a> timer_interrupt</h3><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>timer_interrupt:
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>首先，它保存了当前的数据段寄存器 <code>ds</code>、<code>es</code>、<code>fs</code> 到堆栈中，以便后续恢复。</p><p>接着，它将 <code>edx</code>、<code>ecx</code>、<code>ebx</code>、<code>eax</code> 寄存器的值依次压入堆栈中，保存了这些寄存器的内容。</p><p>然后，它将 <code>ds</code> 和 <code>es</code> 寄存器设置为指向内核数据段，即将数据段选择符设置为 <code>0x10</code>，以确保后续操作在内核数据段中进行。</p><p><code>fs</code> 寄存器被设置为指向局部数据段，即设置数据段选择符为 <code>0x17</code>，这可能是为了在中断处理中访问特定于定时器的数据。</p><p><code>incl jiffies</code> 增加了全局变量 <code>jiffies</code>，它用于跟踪系统运行时间。</p><p>接下来，它向中断控制器发送 <code>End-of-Interrupt（EOI）</code>信号，以告知硬件中断处理已完成。</p><p>然后，它从堆栈中取出当前特权级别（CPL），并将其压入堆栈，作为参数传递给 <code>do_timer()</code> 函数。<code>do_timer()</code>函数负责执行任务切换、计时等工作。</p><p>最后，它调用 <code>do_timer()</code> 函数，并通过 <code>ret_from_sys_call</code> 返回到系统调用的返回路径。</p><h3 id="hd-interrupt" tabindex="-1"><a class="header-anchor" href="#hd-interrupt" aria-hidden="true">#</a> hd_interrupt</h3><div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code>hd_interrupt:
	pushl %eax
	pushl %ecx
	pushl %edx
	push %ds
...
1:	outb %al,$0x20              # 送主8259A中断控制器EOI命令(结束硬件中断)
	...
	popl %eax
	iret
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="floppy-interrupt" tabindex="-1"><a class="header-anchor" href="#floppy-interrupt" aria-hidden="true">#</a> floppy_interrupt</h3><p>floppy_interrupt: pushl %eax pushl %ecx pushl %edx push %ds push %es push %fs movl $0x10,%eax mov %ax,%ds mov %ax,%es movl $0x17,%eax mov %ax,%fs movb $0x20,%al outb %al,$0x20 # EOI to interrupt controller #1</p><h1 id="do-floppy为一函数指针-将被赋值实际处理c函数指针。该指针在被交换放到eax寄存器后" tabindex="-1"><a class="header-anchor" href="#do-floppy为一函数指针-将被赋值实际处理c函数指针。该指针在被交换放到eax寄存器后" aria-hidden="true">#</a> do_floppy为一函数指针，将被赋值实际处理C函数指针。该指针在被交换放到eax寄存器后</h1><h1 id="就将do-floppy变量置空。然后测试eax中原指针是否为空-若是则使指针指向c函数。" tabindex="-1"><a class="header-anchor" href="#就将do-floppy变量置空。然后测试eax中原指针是否为空-若是则使指针指向c函数。" aria-hidden="true">#</a> 就将do_floppy变量置空。然后测试eax中原指针是否为空，若是则使指针指向C函数。</h1><pre><code>xorl %eax,%eax
xchgl do_floppy,%eax
testl %eax,%eax
jne 1f

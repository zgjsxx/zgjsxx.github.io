import{_ as n,V as s,W as a,a0 as t}from"./framework-9a29aaa0.js";const p={},e=t(`<h1 id="workflow-线程池分析" tabindex="-1"><a class="header-anchor" href="#workflow-线程池分析" aria-hidden="true">#</a> workflow 线程池分析</h1><p>线程池是日常开发中很常用的一种管理线程的工具。它是<strong>池化技术</strong>中的一种。</p><p>池化技术的初衷就是将一些资源进行重复利用，来避免重复的构建来提高执行效率。类似的还有数据库连接池，字符串常量池，httpClient 连接池。</p><p>本文将分享一个好用的线程池，其来源于搜狗开源高性能网络框架workflow。</p><p>workflow 是搜狗公司近期开源发布的一款 C++ 服务器引擎，支撑搜狗几乎所有后端 C++ 在线服务，包括所有搜索服务，云输入法，在线广告等，每日处理超百亿请求。</p><p>下面就通过阅读源码的方式来深入了解workflow线程池的实现原理。</p><h2 id="workflow线程池" tabindex="-1"><a class="header-anchor" href="#workflow线程池" aria-hidden="true">#</a> workflow线程池</h2><p>workflow的线程池主要分布在四个文件中，msgqueue.h/msgqueue.c/thrdpool.h/thrdpool.c。</p><p>首先我们分析msgqueue.h/msgqueue.c</p><h3 id="msgqueue" tabindex="-1"><a class="header-anchor" href="#msgqueue" aria-hidden="true">#</a> msgqueue</h3><p>从名字中得知，这个是一个消息队列。这个消息队列中盛放的就是需要在线程池中执行的任务。所谓<strong>任务</strong>就是一个执行函数和一个对应的入参。</p><p>在workflow中，任务的定义是thrdpool_task，有两个参数，第一个是一个函数指针routine，第二个参数是context上下文。</p><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">struct</span> <span class="token class-name">thrdpool_task</span>
...
<span class="token punctuation">}</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>接下来看一下msgqueue的定义。 其中包含了两个队列。一个队列用于放置新任务，一个队列用于拿取新任务。当get的任务队列为空时，就将get队列和put队列进行切换。如果get和put共用一个队列，那么放置任务和取任务都需要加锁，而使用两个队列的好处是只有当get的任务队列为空时进行切换时，才需要进行加锁。</p><p><code>__msgqueue</code>结构体的定义中，get_head就是读队列的队头，put_head是放置队列的队头，put_tail是放置队列的队尾。</p><p>msg_max代表最大能够放置的任务数量。</p><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">struct</span> <span class="token class-name">__msgqueue</span>
<span class="token punctuation">{</span>
...
<span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">endif</span></span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>因为其实c语言编写的代码，为了使其可以被c和c++程序都调用，因此在代码中使用了<code>extern &quot;C&quot;</code>。</p><p>msgqueue的头文件中提供了6个方法，其作用总结如下：</p><ul><li>msgqueue_create：创建msgqueue。</li><li>msgqueue_put：放置任务到msgqueue中。</li><li>msgqueue_get：从msgqueue中取出任务。</li><li>msgqueue_set_nonblock：将msgqueue设置为nonblock。</li><li>msgqueue_set_block：将msgqueue设置为block。</li><li>msgqueue_destroy：销毁msgqueu额。</li></ul><p>上述接口做到了见文知意，值得学习。</p><p>下面看看这些接口是如何实现的。</p><h4 id="msgqueue-create" tabindex="-1"><a class="header-anchor" href="#msgqueue-create" aria-hidden="true">#</a> msgqueue_create</h4><p>第一眼看到这个代码，居然是这种梯形的代码，难以说是优美的，不知道是不是我没有领悟到其中的精髓？这个梯形的代码应该可以使用if-return进行优化。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>	ret = pthread_mutex_init(&amp;queue-&gt;get_mutex, NULL);
	if (ret != 0)
	{
	}
	ret = pthread_mutex_init(&amp;queue-&gt;put_mutex, NULL);
	if (ret != 0) 
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token class-name">msgqueue_t</span> <span class="token operator">*</span><span class="token function">msgqueue_create</span><span class="token punctuation">(</span><span class="token class-name">size_t</span> maxlen<span class="token punctuation">,</span> <span class="token keyword">int</span> linkoff<span class="token punctuation">)</span>
<span class="token punctuation">{</span>
	...
	<span class="token keyword">return</span> <span class="token constant">NULL</span><span class="token punctuation">;</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h4 id="msgqueue-put" tabindex="-1"><a class="header-anchor" href="#msgqueue-put" aria-hidden="true">#</a> msgqueue_put</h4><h4 id="msgqueue-get" tabindex="-1"><a class="header-anchor" href="#msgqueue-get" aria-hidden="true">#</a> msgqueue_get</h4><h4 id="msgqueue-set-nonblock" tabindex="-1"><a class="header-anchor" href="#msgqueue-set-nonblock" aria-hidden="true">#</a> msgqueue_set_nonblock</h4><h4 id="msgqueue-set-block" tabindex="-1"><a class="header-anchor" href="#msgqueue-set-block" aria-hidden="true">#</a> msgqueue_set_block</h4><h4 id="msgqueue-destroy" tabindex="-1"><a class="header-anchor" href="#msgqueue-destroy" aria-hidden="true">#</a> msgqueue_destroy</h4><h3 id="thrdpool" tabindex="-1"><a class="header-anchor" href="#thrdpool" aria-hidden="true">#</a> thrdpool</h3><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">typedef</span> <span class="token keyword">struct</span> <span class="token class-name">msg_t</span> <span class="token punctuation">{</span>
...
<span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,50),o=[e];function c(l,i){return s(),a("div",null,o)}const r=n(p,[["render",c],["__file","workflow-threadpool.html.vue"]]);export{r as default};

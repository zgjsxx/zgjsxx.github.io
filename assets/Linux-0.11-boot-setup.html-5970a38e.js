import{_ as s,V as a,W as l,X as e,Y as i,$ as t,a0 as n,F as r}from"./framework-9a29aaa0.js";const c={},v=n(`<h1 id="linux-0-11-boot目录setup-s详解" tabindex="-1"><a class="header-anchor" href="#linux-0-11-boot目录setup-s详解" aria-hidden="true">#</a> Linux-0.11 boot目录setup.s详解</h1><h2 id="模块简介" tabindex="-1"><a class="header-anchor" href="#模块简介" aria-hidden="true">#</a> 模块简介</h2><p>setup.s用于加载操作系统的一些信息，其主要处理了如下一些事情：</p><ul><li>打印硬件信息</li><li>重新搬运system的位置</li><li>设置IDT和GDT</li><li>打开A20地址线</li><li>切换32位保护模式</li><li>跳转到system.s中运行</li></ul><h2 id="过程详解" tabindex="-1"><a class="header-anchor" href="#过程详解" aria-hidden="true">#</a> 过程详解</h2><h3 id="step1-打印硬件信息" tabindex="-1"><a class="header-anchor" href="#step1-打印硬件信息" aria-hidden="true">#</a> step1：打印硬件信息</h3><p>下面是setup.s的30-41行：</p><div class="language-x86asm line-numbers-mode" data-ext="x86asm"><pre class="language-x86asm"><code>entry start
start:
...
! posterity.
	...
	mov	[10],bx
	mov	[12],cx
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里用到的是0x10中断，其主功能码(AH)是<code>12h</code>，次功能码<code>10h</code>。其输出如下：</p><ul><li>BH：显示状态(0x00，彩色模式， I/O端口 3Dxh， 0x01， 单色模式， I/O端口 3bxh)</li><li>BL： 安装的显示内存(00h = 64k， 01h = 128K， 02h = 192k，03h = 256k)</li><li>CH：特征连接器位</li><li>CL：切换设置</li></ul>`,26),m={href:"https://www.youtube.com/watch?v=FpnGzLfIoz4&ab_channel=%E9%9F%B3%E4%B9%90%E7%A7%81%E8%97%8F%E9%A6%86",target:"_blank",rel:"noopener noreferrer"},o=n(`<p>最终在获取完这些信息之后，将其<code>AH</code>存储在了<code>0x90008</code>(<code>AH</code>返回新的BIOS移除了该返回值),将安装的显示内存存储在了<code>0x9000a</code>,将显卡的特性参数存储在了<code>0x9000c</code>。</p><p>下面是setup.s的64-74行，这里用于获取第一块硬盘的信息。</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>! Get hd0 data

import{_ as s,V as i,W as l,Z as e,X as n,Y as d,a0 as a}from"./framework-c954d91f.js";const t={},c=n("h1",{id:"大模型的核心-竟然只是矩阵乘法",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#大模型的核心-竟然只是矩阵乘法","aria-hidden":"true"},"#"),d(" 大模型的核心，竟然只是矩阵乘法？")],-1),p=n("p",null,"今天我们经常听到这些词：",-1),r=n("p",null,"大模型、Transformer、Attention、GPU、CUDA、算力、训练、推理。",-1),u=n("p",null,"听起来都很高深。",-1),o=n("p",null,"但如果把这些概念一层一层拆开，你会发现一个很有意思的事实：",-1),v=n("p",null,[n("strong",null,"现代 AI 的核心计算，大量都是矩阵运算。")],-1),m=n("p",null,"尤其是大语言模型 LLM，不管是前向推理，还是训练时的反向传播，本质上都离不开矩阵乘法。",-1),g=n("p",null,"而 GPU 之所以能成为 AI 时代的核心硬件，也是因为它特别擅长做大规模并行矩阵计算。",-1),b=a(`<figure><img src="https://cdn.nlark.com/yuque/0/2026/png/26683070/1779690681397-88604670-8368-4fbe-ac85-fbea5d05e1a7.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><h2 id="一、文本进入模型后-先变成矩阵" tabindex="-1"><a class="header-anchor" href="#一、文本进入模型后-先变成矩阵" aria-hidden="true">#</a> 一、文本进入模型后，先变成矩阵</h2><p>神经网络不能直接理解文字。</p><p>比如一句话：</p><div class="language-latex line-numbers-mode" data-ext="latex"><pre class="language-latex"><code>我 爱 人工智能
...
高单核性能
        ...
        + ...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>矩阵 C 有成千上万个元素，每个元素都可以分配给不同线程去算。</p><p>所以 GPU 的优势不是一个人算完整张表，而是一群线程同时算很多格子。</p>`,20),A=a(`<figure><img src="https://cdn.nlark.com/yuque/0/2026/png/26683070/1779690933795-2a0e7a6c-ccda-4c73-b5d6-7b3c01e225f1.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><h2 id="七、cuda-怎么组织-gpu-并行" tabindex="-1"><a class="header-anchor" href="#七、cuda-怎么组织-gpu-并行" aria-hidden="true">#</a> 七、CUDA 怎么组织 GPU 并行？</h2><p>在 CUDA 里，程序不是直接说“让 GPU 帮我算”。</p><p>它会把任务切成很多层：</p><div class="language-latex line-numbers-mode" data-ext="latex"><pre class="language-latex"><code>Grid
 ├── Block
 │    ├── Thread
 │    ├── Thread
 │    └── ...
...
数据要搬多少次？
数据能不能被复用？
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>GPU 上的 global memory，也就是显存，容量很大，但访问速度相对慢。</p><p>而 SM 内部的 shared memory 很快，但容量很小。</p>`,10),C=a(`<figure><img src="https://cdn.nlark.com/yuque/0/2026/png/26683070/1779691269294-a9f112c0-5029-4ea0-a87e-98dbae3b43f4.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>如果每个线程都反复从 global memory 读取 A 和 B 的元素，性能会被内存访问拖慢。</p><p>所以 CUDA 优化矩阵乘法时，经常会使用一个核心技巧：</p><div class="language-latex line-numbers-mode" data-ext="latex"><pre class="language-latex"><code>Tile 分块计算
...
怎么少搬数据？
怎么把 GPU 喂饱？
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>大模型看起来像是在“理解语言”。</p><p>但从硬件视角看，它其实是在一层又一层地做矩阵乘法。</p><p>智能的表象背后，是海量矩阵、海量数据搬运，以及 GPU 上成千上万个线程的并行计算。</p><p><strong>当你真正理解了矩阵乘法，你就已经摸到了现代 AI 计算的地基。</strong></p><p><strong>当你真正理解了 Tile 分块，你就已经拿到了理解 GPU 加速的第一把钥匙。</strong></p>`,21);function G(W,M){return i(),l("div",null,[c,p,r,u,o,v,m,g,e(" 这是一张图片，ocr 内容为： "),b,e(" 这是一张图片，ocr 内容为： "),x,e(" 这是一张图片，ocr 内容为： "),h,e(" 这是一张图片，ocr 内容为： "),k,e(" 这是一张图片，ocr 内容为： "),f,e(" 这是一张图片，ocr 内容为： "),_,e(" 这是一张图片，ocr 内容为： "),A,e(" 这是一张图片，ocr 内容为： "),y,e(" 这是一张图片，ocr 内容为： "),C,e(" 这是一张图片，ocr 内容为： "),B,e(" 这是一张图片，ocr 内容为： "),U,e(" 这是一张图片，ocr 内容为： "),P,e(" 这是一张图片，ocr 内容为： "),X])}const q=s(t,[["render",G],["__file","大模型的核心，竟然只是矩阵乘法？.html.vue"]]);export{q as default};
